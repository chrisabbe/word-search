/* WORD SEARCH ENGINE — STABLE BASELINE (matches your original working logic) */

const { title, subtitle, gridRows, gridCols, seed: INITIAL_SEED, words: WORDS, storageKey: LS_KEY } = PUZZLE_CONFIG;
...
function generatePuzzle(){
  [...WORDS].map(w=>w.toUpperCase()).sort((a,b)=>b.length-a.length).forEach(w=>placeWord(w));
...
function scaleGridToFit(){
  ...
  const gridWidth  = gridRect.width || (gridCols * 28);
...
function renderWordList(){
  ...
  WORDS.forEach(w=>{
    const div = document.createElement("div");
    div.className = "word" + (foundWords.has(w.toUpperCase()) ? " found" : "");
...
/* ========= CLEAR BUTTON (ORIGINAL BEHAVIOR) ========= */
