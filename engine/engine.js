/* WORD SEARCH ENGINE — RESPONSIVE GRID (PIXEL LOCK, TAP + DRAG) */
...
function applyGridSizing(){
  ...
  const CELL = isMobile ? 28 : 32;
  const GAP = 2; // must match --gap in CSS

  gridEl.style.gridTemplateColumns = `repeat(${gridCols}, ${CELL}px)`;
  gridEl.style.width = (gridCols * CELL + (gridCols - 1) * GAP) + "px";
}
...
function renderGrid(){
  ...
  for(let r=0;r<gridRows;r++){
    for(let c=0;c<gridCols;c++){
      ...
      d.className = "cell";
      d.textContent = grid[r][c];     // ✅ single source of letter rendering
      d.dataset.r = r;
      d.dataset.c = c;
...
gridEl.addEventListener("click",e=>{
  ...
  if(!cell) return;
  if(!tapStart){
    tapStart=cell; startCell=cell; previewTo(cell);
  } else {
    startCell=tapStart; previewTo(cell); commit();
    clearPreview(); tapStart=null; startCell=null;
  }
});
...
/* Re-apply sizing on rotation / resize (important for Android Chrome) */
window.addEventListener("resize", () => setTimeout(() => {
  renderGrid();   // re-renders with correct pixel sizing
  applyFound();
}, 50));
