/* WORD SEARCH ENGINE — STABLE BASELINE WITH TAP + DRAG SUPPORT (FIXED PORTRAIT CLIP + IFRAME SCROLL) */
...
/* IMPORTANT: allow iframe/page vertical scrolling when finger starts on grid */
...
/* ========= DRAG (with scroll-friendly touch-action switching) ========= */
gridEl.addEventListener("pointerdown",e=>{
  ...
  /* While dragging, prevent browser scroll/gesture conflicts */
  gridEl.style.touchAction = "none";

  /* Capture pointer so dragging stays stable even if finger moves fast */
...
/* ========= RESPONSIVE CELL SIZING (NO CLIPPING) =========
   We measure actual rendered border width and include it in the math.
*/
function resizeCellsToFit(){
  ...
  // Must match your CSS gap (2px). If you ever change --gap, change here too.
  const gap = 2;

  // Measure border width from an actual cell (prevents the “cut off” issue)
  let borderEachSide = 1; // fallback
  if (cellEls && cellEls[0]) {
    const cs = getComputedStyle(cellEls[0]);
    const bl = parseFloat(cs.borderLeftWidth) || 0;
    const br = parseFloat(cs.borderRightWidth) || 0;
    borderEachSide = (bl + br) / 2 || 1;
  }
  ...
  const size = Math.floor(
    (maxWidth - (gridCols - 1) * gap - safety) / gridCols
    - borderTotalPerCell
  );
